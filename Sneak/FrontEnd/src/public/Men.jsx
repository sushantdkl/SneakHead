...
// Men's category page with filtering and product listing
const Men = () => {
  ...
  const fetchMenProducts = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3000/api/products?category=men');
      if (response.ok) {
  ...
  const handleAddToCart = async (product) => {
    ...
    try {
      ...
      console.log('Adding to cart:', { userId: userData.id, productId: product.id, token: token ? 'present' : 'missing' });
  ...
  return (
    <div className="min-h-screen bg-sneakhead-black text-white">
      ...
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-sneakhead-black via-sneakhead-gray to-sneakhead-black py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-6xl font-black font-grotesk mb-4">
            MEN'S <span className="text-sneakhead-red">COLLECTION</span>
          </h1>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">
            Discover the latest trends in men's sneakers and athletic wear
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <select
            ...
            className="px-4 py-3 bg-sneakhead-black border border-sneakhead-light-gray rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-sneakhead-red [&>option]:bg-sneakhead-black [&>option]:text-white"
          >
